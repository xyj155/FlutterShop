...
class _MyHomePageState extends State<PartTimeJobPage> {
    ...
    return new DefaultDropdownMenuController(
      ...
      onSelected: ({int menuIndex, int index, int subIndex, dynamic data}) {
        ...
        switch (menuIndex) {
          ...
          case 2:
            setState(() {
              PARAM_JOBKIND = JOB_LIST[index]['children'][subIndex]["id"].toString();
              getPartTimeJobByPage(
                  PARAM_LOCATION, PARAM_JOBTYPE, PARAM_JOBKIND);
            });
            break;
  ...
  Widget setJobItem(BuildContext buildContext, int position) {
    return new Container(
      margin: EdgeInsets.only(top: screenUtils.setWidgetHeight(10)),
      color: Colors.white,
      padding: EdgeInsets.only(
      ...
      child: new Column(
        ...
        children: <Widget>[
          ...
          new Container(
            padding: EdgeInsets.only(bottom: screenUtils.setWidgetHeight(8)),
            child: new Text(
              _job_list[position].jobShop,
              style: new TextStyle(
                  color: Colors.black54, fontSize: screenUtils.setFontSize(15)),
            ),
          ),
          new Row(
            children: <Widget>[
